...
tabletest = [['STEPS','MSE: Mean','MSE: StdDev','R^2: Mean','R^2: StdDev'],
         ['A', round(np.mean(mse),2), round(np.std(mse),2), round(np.mean(r2),2), round(np.std(r2),2)]]
...
<tr><td>A    </td><td>32.09    </td><td>5.04       </td><td>0.88     </td><td>0.02       </td></tr>
</tbody>
</table>
...
```
    ...
    Time taken to complete iteration: 33.17 minutes
...
```
    ...
    The predicted concrete strength is 59.37
